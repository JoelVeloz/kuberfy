...
type State = { status: "loading" } | { status: "unauthorized" } | { status: "not-found" } | { status: "error" } | { status: "ready"; name: string; apps: AppRow[] };
  ...
  React.useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setState({ status: "ready", name: project.name, apps: withStatus });
      } catch (err) {
        if (cancelled) return;
