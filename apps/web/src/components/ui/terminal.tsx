...
export const Terminal = ({
  ...
  const content = (
    <div ref={containerRef} className={cn("z-0 w-full max-w-lg border border-white/10 bg-black", className)}>
      <div className="flex items-center gap-1.5 border-b border-white/10 px-4 py-2.5">
        <span className="bg-success size-1.5 rounded-full" />
        <span className="font-mono text-[11px] text-neutral-500">deploy</span>
      </div>
      <pre className="p-4">
        <code className="grid gap-y-1.5 overflow-auto text-[13px] text-neutral-400">{wrappedChildren}</code>
      </pre>
    </div>
  )
