...
export function DatabaseConnectionCard({ app }: { app: ApiApplicationDetail }) {
  ...
  const line =
    format === "env"
      ? { label: connection.envName, display: `${connection.envName}=${connection.url(revealed)}`, value: `${connection.envName}=${connection.url(true)}` }
      : { label: "Connection string", display: connection.url(revealed), value: connection.url(true) };
