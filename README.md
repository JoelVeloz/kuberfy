...
The command above is the whole install. When executed, it prompts interactively for your admin email; Kuberfy is reached by the server's bare IP right after install, with HTTPS on a real domain configurable anytime afterward from the dashboard's Settings page (no restart needed). For non-interactive automated installs, you can pass the admin email as an environment variable:
...
For local development instead of a real server, copy `.env.example` to `.env` and run `docker compose up --build` — the same image, without Swarm for Kuberfy itself. Deployed applications are still created as Swarm services (same as production), so your local Docker daemon needs Swarm mode active once (`docker swarm init`) before deploying an app from the dashboard.

## Exposed ports

Only 80 and 443 (Traefik) are open by default — everything, including the dashboard itself, is reached through Traefik on those ports, over the server's bare IP right after install and over a real domain with HTTPS once one is configured from Settings. The panel's own port 3000 is reserved by the installer but **not** published; it stays off unless you turn on "direct access" from the dashboard's Settings page (an emergency bypass in case Traefik itself is ever broken), and that toggle takes effect immediately, no restart needed. The same Settings page lists every port Docker currently has published on the host, read live rather than hand-maintained.
