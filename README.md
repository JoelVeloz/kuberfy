...
| `KUBERFY_DOMAIN`  | no       | Public domain routed to Kuberfy via Traefik. Optional — when omitted, you're asked whether to use a domain (a free `sslip.io` hostname with a random token, not derived from anything guessable, with real HTTPS) or the server's bare IP with no HTTPS. Only used for this first setup — change it later from the dashboard's Settings page, no restart needed. |
| `KUBERFY_IP_ONLY` | no       | Set to `1` to skip the domain prompt and use the server's bare IP with no HTTPS, non-interactively                                                                                                                                                                                                                                                               |
